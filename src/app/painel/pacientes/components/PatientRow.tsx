      ...
      >
        ...
        <div className='md:w-[20%] hidden md:flex items-center py-3 px-4'>
          <span className="text-sm">
            {item.cliente_id ? item.cliente_id.replace(/^55/, '') : 'Não informado'}
          </span>
        </div>

        {/* Agendamentos */}
        <div className='md:w-[45%] hidden md:flex items-center py-3 px-4'>
          {item.agendamentos && item.agendamentos.length > 0 ? (
            <div className="flex flex-col gap-1">
              {item.agendamentos.slice(0, 2).map((agendamento, index) => (
                <div key={index} className="flex items-center gap-1">
                  <Calendar className="h-3 w-3 text-gray-500" />
                  <span className="text-xs">
                    {format(new Date(agendamento.data_agendamento), "dd/MM/yyyy")} às {agendamento.hora_agendamento.substring(0, 5)}
                  </span>
                </div>
              ))}
              {item.agendamentos.length > 2 && (
                <span className="text-xs text-gray-500">+{item.agendamentos.length - 2} agendamento(s)</span>
              )}
            </div>
          ) : (
            <span className="text-sm text-gray-500">Sem agendamentos</span>
          )}
        </div>
