import { AssociadoRow } from './AssociadoRow';
...
export function AssociadosTable({ 
  items,
  expandedItem,
  onItemExpand,
  fetchNextPage,
    ...
    >
      {items.map((associado) => (
        <AssociadoRow 
          key={associado.id} 
          item={associado}
          isExpanded={expandedItem === associado.id}
          onExpand={() => onItemExpand?.(associado.id)}
        />
      ))}
      {isFetchingNextPage && (
